...
Page({
  ...
  async getVideoGroupListData() {
    ...
    this.setData({
      videGroupList: videGroupListData.data.slice(0, 14),
  ...
  // 点击切换导航的回调
  changeNav(event) {
    let navId = event.currentTarget.id//通过id向event传参的时候如果number会自动转成字串串
    this.setData({
      navId: navId >>> 0
    })
    //显示正在加载
  ...
  handlePlay(event) {
    ...
    // 判断当前的视频之前是否播放过，是否有播放记录，如果有，跳转至指定的位置
  ...
  handleEnded(event) {
    ...
    //移除记录播放时长数组中当前视频的对象
    let { videoUpdateTime } = this.data
    videoUpdateTime.splice(videoUpdateTime.findIndex(item => item.id === event.currentTarget.id))
    this.setData({
      videoUpdateTime
    })
  },
